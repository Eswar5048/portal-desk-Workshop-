...
def tenure_factor(tenure_years: int) -> float:
    ...
    if tenure_years not in TENURE_FACTORS:
        raise PricingError(f"Tenure must be 1, 2 or 3 years (got {tenure_years})")
...
def add_on_factor(product: ProductCode, add_ons: list[str]) -> float:
    ...
    factor = 1.0
    available = ADD_ONS.get(product, {})
    for code in add_ons:
        c = code.strip().upper()
        if not c:
            continue
        if c not in available:
            raise PricingError(f"Add-on '{c}' is not available for {product.name if hasattr(product, 'name') else product}")
        factor += available[c]
    return factor
...
def calculate_premium(
    *,
    ...
    return round(float(max(premium, MIN_PREMIUM)), 2)
